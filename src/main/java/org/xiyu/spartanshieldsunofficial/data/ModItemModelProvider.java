...
public class ModItemModelProvider extends ItemModelProvider {
    ...
    protected ResourceLocation createBasicShieldModel(Item itemIn) {
        ...
        ModelFile blockingModel = this.withExistingParent(itemPath + "_blocking", ResourceLocation.tryBuild(ModSpartanShields.ID, "basic_shield_base_blocking"))
                .texture("layer0", "item/" + itemPath);
        return this.withExistingParent(itemPath, ResourceLocation.tryBuild(ModSpartanShields.ID, "basic_shield_base"))
                .texture("layer0", "item/" + itemPath)
    ...
    protected ResourceLocation createMekanismPoweredBasicShieldModel(Item itemIn) {
        String itemPath = BuiltInRegistries.ITEM.getKey(itemIn).getPath();
        ModelFile blockingModel = this.getExistingFile(ResourceLocation.tryBuild(ModSpartanShields.ID, "mekanists_basic_shield_base_blocking"));
        ModelFile deactivatedModel = this.getExistingFile(ResourceLocation.tryBuild(ModSpartanShields.ID, "mekanists_basic_shield_base_deactivated"));
        ModelFile deactivatedBlockingModel = this.getExistingFile(ResourceLocation.tryBuild(ModSpartanShields.ID, "mekanists_basic_shield_base_deactivated_blocking"));
        ResourceLocation disabled = ResourceLocation.parse("disabled");
        ResourceLocation blocking = ResourceLocation.parse("blocking");
        return this.withExistingParent(itemPath, ResourceLocation.tryBuild(ModSpartanShields.ID, "mekanists_basic_shield_base"))
    ...
    protected ResourceLocation createEnderIOPoweredBasicShieldModel(Item itemIn) {
        ...
        ModelFile deactivatedModel = this.withExistingParent(itemPath + "_deactivated", ResourceLocation.tryBuild(ModSpartanShields.ID, "basic_shield_base"))
                .texture("layer0", baseTexture).texture("layer1", deactivatedTexture);
        ModelFile deactivatedBlockingModel = this.withExistingParent(itemPath + "_deactivated_blocking", ResourceLocation.tryBuild(ModSpartanShields.ID, "basic_shield_base_blocking"))
                .texture("layer0", baseTexture).texture("layer1", deactivatedTexture);
        ResourceLocation disabled = ResourceLocation.parse("disabled");
        ResourceLocation blocking = ResourceLocation.parse("blocking");
        return this.withExistingParent(itemPath, ResourceLocation.tryBuild(ModSpartanShields.ID, "basic_shield_base"))
    ...
    protected ResourceLocation createTowerShieldModel(Item itemIn) {
        String itemPath = BuiltInRegistries.ITEM.getKey(itemIn).getPath();
        ModelFile blockingModel = this.getExistingFile(ResourceLocation.tryBuild(ModSpartanShields.ID, "tower_shield_base_blocking"));
        return this.withExistingParent(itemPath, ResourceLocation.tryBuild(ModSpartanShields.ID, "tower_shield_base"))
                .override().predicate(ResourceLocation.parse("blocking"), 1.0f).model(blockingModel).end()
                .getLocation();
    }
