...
public class ModItemModelProvider extends ItemModelProvider 
{
	...
	protected void registerModels() 
	{
		...
		// EnderIO Shields
		createEnderIOPoweredBasicShieldModel(ModItems.DARK_STEEL_RIOT_BASIC_SHIELD.get());
	...
	protected ResourceLocation createBasicShieldModel(Item itemIn)
	{
		...
		ModelFile blockingModel = withExistingParent(itemPath + "_blocking", ResourceLocation.fromNamespaceAndPath(ModSpartanShields.ID, "basic_shield_base_blocking"))
				.texture("layer0", "item/" + itemPath);
		return withExistingParent(itemPath, ResourceLocation.fromNamespaceAndPath(ModSpartanShields.ID, "basic_shield_base"))
				.texture("layer0", "item/" + itemPath)
	...
	{
		String itemPath = BuiltInRegistries.ITEM.getKey(itemIn).getPath();
		ModelFile blockingModel = getExistingFile(ResourceLocation.fromNamespaceAndPath(ModSpartanShields.ID, "mekanists_basic_shield_base_blocking"));
		ModelFile deactivatedModel = getExistingFile(ResourceLocation.fromNamespaceAndPath(ModSpartanShields.ID, "mekanists_basic_shield_base_deactivated"));
		ModelFile deactivatedBlockingModel = getExistingFile(ResourceLocation.fromNamespaceAndPath(ModSpartanShields.ID, "mekanists_basic_shield_base_deactivated_blocking"));
		ResourceLocation disabled = ResourceLocation.parse("disabled");
		ResourceLocation blocking = ResourceLocation.parse("blocking");
		return withExistingParent(itemPath, ResourceLocation.fromNamespaceAndPath(ModSpartanShields.ID, "mekanists_basic_shield_base"))
	...
	protected ResourceLocation createEnderIOPoweredBasicShieldModel(Item itemIn)
	{
		...
		ModelFile deactivatedModel = withExistingParent(itemPath + "_deactivated", ResourceLocation.fromNamespaceAndPath(ModSpartanShields.ID, "basic_shield_base"))
				.texture("layer0", baseTexture).texture("layer1", deactivatedTexture);
		ModelFile deactivatedBlockingModel = withExistingParent(itemPath + "_deactivated_blocking", ResourceLocation.fromNamespaceAndPath(ModSpartanShields.ID, "basic_shield_base_blocking"))
				.texture("layer0", baseTexture).texture("layer1", deactivatedTexture);
		ResourceLocation disabled = ResourceLocation.parse("disabled");
		ResourceLocation blocking = ResourceLocation.parse("blocking");
		return withExistingParent(itemPath, ResourceLocation.fromNamespaceAndPath(ModSpartanShields.ID, "basic_shield_base"))
	...
	{
		String itemPath = BuiltInRegistries.ITEM.getKey(itemIn).getPath();
		ModelFile blockingModel = getExistingFile(ResourceLocation.fromNamespaceAndPath(ModSpartanShields.ID, "tower_shield_base_blocking"));
		return withExistingParent(itemPath, ResourceLocation.fromNamespaceAndPath(ModSpartanShields.ID, "tower_shield_base"))
				.override().predicate(ResourceLocation.parse("blocking"), 1.0f).model(blockingModel).end()
				.getLocation();
	}
