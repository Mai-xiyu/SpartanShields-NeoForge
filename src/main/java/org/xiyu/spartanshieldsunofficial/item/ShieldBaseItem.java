...
public class ShieldBaseItem extends ShieldItem {
    ...
    public void addEffectsTooltip(ItemStack stack, Item.TooltipContext context, List<Component> tooltip, TooltipFlag flagIn) {
        // Get payback enchantment level
        // Note: In Forge, TooltipContext doesn't have level() method directly
        // We'll skip this check for now as the payback damage can still be displayed
        int paybackLevel = 0;
        float paybackDamage = ModDataComponents.getPaybackDamage(stack);
        if (paybackDamage > 0) {
